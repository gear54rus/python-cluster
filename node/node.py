        ...
        while var:
            ...
            if msg['type'] == 'Start':
                if self.status == 'ready to start':
                    ...
                    self.connection.sendMessage('Finished',datetime.datetime.now() + ',' + self.result)
                else:
                    self.connection.sendMessage('Reject','Node dont ready to start. Node status: ' + self.status)
